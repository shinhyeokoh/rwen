...
from .korean import normalize, jamo_to_korean
...
class TextProcessing(object):
    def __init__(self, symbol_set, cleaner_names, p_arpabet=0.0,
        ...
        self.backend = EspeakBackend(language="en-us", preserve_punctuation=True, with_stress=True)
    ...
    def clean_text(self, text, backend):
    ...
    def encode_text(self, text, return_all=False):
        ...
        text_clean = [self.clean_text(split, self.backend) if split[0] != '{' else split
                      for split in _arpa_re.findall(text)]
        word_level_lens = [len(t) for t in text_clean]
        ...
        return text_encoded, word_level_lens