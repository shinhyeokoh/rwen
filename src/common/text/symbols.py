...
def get_symbols(symbol_set='english_basic'):
    ...
    elif symbol_set == "english_phonemized":
        _pad = '_'
        _punctuation = '();:,.!?¡¿—…"«»“” '
        _special = '-'
        _letters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'
        _letters_ipa = "ɑɐɒæɓʙβɔɕçɗɖðʤəɘɚɛɜɝɞɟʄɡɠɢʛɦɧħɥʜɨɪʝɭɬɫɮʟɱɯɰŋɳɲɴøɵɸθœɶʘɹɺɾɻʀʁɽʂʃʈʧʉʊʋⱱʌɣɤʍχʎʏʑʐʒʔʡʕʢǀǁǂǃˈˌːˑʼʴʰʱʲʷˠˤ˞↓↑→↗↘'̩'ᵻ"
        # _special_tokens = ['[NONE]', '[PAR]', '[CHI]', '[SIB]']
        symbols = list(_pad + _special + _punctuation + _letters + _letters_ipa) + _arpabet #+ _special_tokens
    else:
...
def get_pad_idx(symbol_set='english_basic'):
    if symbol_set in {'english_basic', 'english_basic_lowercase', 'korean', 'english_phonemized'}:
        return 0
    else:
